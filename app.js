(function(){
"use strict";
const DATA=window.PARLA_CURRICULUM;
const KEY="parla_v1_1";
const OLD_KEY="parla_v1";
...
const norm=x=>String(x||"").toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[’']/g,"'").replace(/[.,!?;:]/g,"").replace(/\s+/g," ");
...
function fresh(){
  ...
  return{version:"1.1",xp:0,streak:0,last:null,done:{},reviews:{},skills,errors:{},view:"home",legacy:null};
}
function load(){
  let state=fresh();
...
function speak(text,rate=.88){
  ...
  const u=new SpeechSynthesisUtterance(text);u.lang="it-IT";u.rate=rate;
...
function top(active){
  return '<header class="top"><div class="brand"><div class="logo">P</div><div><b>Parla!</b><small>Italiano Coach · A1</small></div></div><nav class="nav">'+
...
function home(){
  ...
  '<section class="card hero"><div class="eyebrow">A1 · Formation progressive</div><h1>Capire. Costruire. Parlare.</h1><p class="lead">Un parcours structuré comme un vrai cours : grammaire utile, conjugaison en contexte, construction de phrases, compréhension et production dès le début.</p><div class="actions"><button class="primary" id="continue">Continuer · '+esc(u.title)+'</button><button class="ghost" data-view="review">Réviser '+dueIds().length+' élément(s)</button></div></section>'+
...
function inputHtml(q){
  ...
  if(q.t==="text")return '<input id="answer" class="answer" autocomplete="off" autocapitalize="sentences" placeholder="Écrivez votre réponse en italien…">';
  if(q.t==="build"){
    builderWords=[];
...
function vocabView(search=""){
  ...
  appEl.innerHTML='<main class="shell">'+top("vocab")+'<section class="card library-head"><div class="eyebrow">Lexique actif A1</div><h1>'+items.length+' mots et expressions.</h1><p class="lead">Le vocabulaire du cours, organisé par contexte et disponible avec prononciation native.</p><input id="vocabSearch" class="search" value="'+esc(search)+'" placeholder="Rechercher en italien ou en français…"></section><section class="library-grid">'+filtered.map(x=>'<article class="card lib-card"><div class="lib-row"><div><div class="eyebrow">Unité '+x.unit+'</div><div class="it">'+esc(x.it)+'</div><p>'+esc(x.fr)+'</p></div><button class="speak" data-speak="'+esc(x.it)+'">🔊</button></div></article>').join("")+'</section></main>';
...
function reviewCard(){
  ...
  if(!reviewQueue.length||reviewIndex>=reviewQueue.length){
    box.innerHTML='<section class="card empty"><div class="eyebrow">Ripasso</div><h2>✓ Révisions à jour</h2><p>Parla fera réapparaître les notions au moment où elles risquent d’être oubliées.</p><button class="ghost" data-view="home">Retour à l’accueil</button></section>';bindNav();return;
  }
...
function dataView(){
  ...
  appEl.innerHTML='<main class="shell">'+top("data")+'<section class="card hero"><div class="eyebrow">Données & confidentialité</div><h2>Votre apprentissage reste chez vous.</h2><p class="lead">Aucun compte, aucun serveur Parla, aucune API payante. La progression est stockée dans le navigateur de cet appareil.</p></section><section class="section"><div class="library-grid"><article class="card lib-card"><h3>Sauvegarde</h3><p>Exportez régulièrement votre progression si vous utilisez Parla comme application installée.</p><div class="backup"><button class="primary" id="exportData">Exporter JSON</button><label class="ghost" style="display:grid;place-items:center">Importer JSON<input id="importData" type="file" accept="application/json" class="hidden"></label></div></article><article class="card lib-card"><h3>Réinitialisation</h3><p>Efface uniquement la progression v1.1 sur cet appareil. L’archive de l’ancienne version n’est pas modifiée.</p><button class="danger" id="resetData" style="margin-top:14px">Réinitialiser v1.1</button></article></div></section><div class="note" style="margin-top:14px">Conseil : Safari peut supprimer le stockage local après une longue période d’inactivité. Un export JSON constitue la meilleure sauvegarde gratuite.</div></main>';
  bindNav();
  $("exportData").onclick=()=>{const blob=new Blob([raw],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="parla-progression-"+localDay()+".json";a.click();URL.revokeObjectURL(url)};
  ...
  $("resetData").onclick=()=>{if(confirm("Réinitialiser toute la progression Parla v1.1 sur cet appareil ?")){localStorage.removeItem(KEY);s=fresh();save();home()}};
}
function render(v){
  s.view=v;save();
  if(v==="home")home();
